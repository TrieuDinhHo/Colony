...
class PreActBottleneck(tfkl.Layer):
    ...
    def __init__(self, cin, cout=None, cmid=None, stride=1, name="preact", **kwargs):
        ...
        self.gn1 = tfkl.GroupNormalization(
            32, epsilon=1e-6,name='gn1')  # TODO check axis
        self.conv1 = conv1x1(cmid, bias=False,name='conv1')
        self.gn2 = tfkl.GroupNormalization(32, epsilon=1e-6,name='gn2')
        # Original code has it on conv1!!
        self.conv2 = conv3x3(cmid, stride, bias=False,name='conv2')
        self.gn3 = tfkl.GroupNormalization(32, epsilon=1e-6,name='gn3')
        self.conv3 = conv1x1(cout, bias=False,name='conv3')

        if (stride != 1 or cin != cout):
            # Projection also with pre-activation according to paper.
            self.downsample = conv1x1(cout, stride, bias=False,name='downsample')

            self.gn_proj = tfkl.GroupNormalization(cout, epsilon=1e-5,name='gn_proj')
...
class ResNetV2(tfk.Model):
    ...
    def __init__(self, block_units, width_factor=1, trainable=True, name="resnet_v2", **kwargs):
        ...
        self.root = tfk.Sequential([
            ...
            tfkl.GroupNormalization(32, epsilon=1e-6),
            tfkl.ReLU()
        ])
