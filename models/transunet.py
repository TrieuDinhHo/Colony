...
class TransUnet():
    ...
    def build_model(self):
        ...
        # Embedding
        if self.hybrid:
            grid_size = self.config.grid #(14,14)
    ...
    def train(self, training_dataset, save_path, epochs=150, batch_size=24, show_history=True):
        ...
        self.model.save(save_path+'/my_model.h5')
        # self.save_model('/content/drive/MyDrive/Data/savekeras/New_Model.h5')
        print(f"Model saved in {save_path}")
        if show_history:
            plt.figure()
            plt.plot(history.history["loss"], label="training loss")
    ...
    def segmentation_loss(y_true, y_pred):
        cce = tfk.losses.BinaryCrossentropy(from_logits=True)
        cross_entropy_loss = cce(y_true=y_true, y_pred=y_pred)
        dice_loss = TransUnet.gen_dice(y_true, y_pred)

        return 0.4 * cross_entropy_loss + 0.6 * dice_loss
    ...
    def gen_dice(y_true, y_pred):
        ...
        # loss += TransUnet.dice_per_class(y_true,y_pred)
